...
`member5` 主要承担前端页面、交互和样式。范围覆盖登录注册、TA 档案、TA 职位列表/详情/申请状态、MO 发布与申请人审核、Admin 页面请求适配，以及后期统一页面 JS 对 `TARecruitment.routes` 和公共请求工具的调用方式。旧 MO 技能匹配页面已下线，因此当前文件清单只保留仍存在的页面与脚本。
...
- 已下线的旧 MO 技能匹配页面、脚本和样式不再出现在当前前端目录。
