...
async function sendInvite(email, role) {
    return TARecruitment.api.request(TARecruitment.routes.admin.invitations(), {
        method: 'POST',
        headers: {
...
async function validateToken(token) {
    return TARecruitment.api.request(TARecruitment.routes.admin.invitationValidation({ token: token }), {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    });
}
...
async function acceptInvite(token, username, password) {
    return TARecruitment.api.request(TARecruitment.routes.admin.invitationAcceptance(), {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Content-Type': 'application/x-www-form-urlencoded'
